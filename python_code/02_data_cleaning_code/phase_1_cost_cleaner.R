...
scraped_itemized_data <- scraped_itemized_data %>%
  ...
  mutate(
    capacity = case_when(
      ...
      q_id == "588" ~ 200,
      q_id == "643J" ~ 169,
      TRUE         ~ capacity    # “else” clause: keep the original
    )
  ) %>% 
...
# some ADNU and network upgrades have MW as total estimated cost , so making them zero
...
scraped_itemized_data <- scraped_itemized_data %>%
  # 2) clean up any extra spaces in your upgrade names
  mutate(
    type_of_upgrade = str_trim(type_of_upgrade)
  ) %>%
  # 3) zero-out only those IDs AND only for ADNU or Distribution Upgrades
  mutate(
    total_estimated_cost = if_else(
      q_id %in% ids_to_zero &
        type_of_upgrade %in% c("ADNU", "Distribution Upgrades"),
      0,
...
# grab just the three columns we need:
proj_meta <- project_summary %>%
  mutate(q_id = as.character(q_id)) %>%
  select(q_id, PTO, project_status) %>%
  rename(status = project_status)

# join into itemized and totals
scraped_itemized_data <- scraped_itemized_data %>%
  left_join(proj_meta, by = "q_id")
...
common_labels <- c(
  ...
  original = "Indicates if the IC phase 1 study had an addendum study",
...
write.csv(scraped_itemized_data,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_project_upgrade_data.csv"),
          row.names = FALSE)

# Save scraped_total_data as CSV
write.csv(scraped_total_data,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_project_type_data.csv"),
          row.names = FALSE)

write.csv(codebook_itemized,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_codebook_project_upgrade_data.csv"),
          row.names = FALSE)

write.csv(codebook_total,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_codebook_project_type_data.csv"),
          row.names = FALSE)
...
base_data <- scraped_itemized_data %>%
  select(q_id, original, cluster, PTO, status, req_deliverability, latitude, longitude, capacity, point_of_interconnection,cost_total_inf_flag) %>%
...
aggregate_data <- aggregated_temp %>%
  ...
  # Add a new variable 'Phase' hard coded as "Phase I" and convert it to a factor
  mutate(Phase = factor(1, levels = c(1))) %>%
...
aggregate_data_wide <- aggregate_data %>%
  pivot_wider(
    id_cols     =        c(
      q_id, original, cluster, status, PTO,
...
# we shall add in cluster, latitude and longitude later
aggregate_data_static <- aggregate_data_wide %>% 
  select(q_id, req_deliverability, capacity, point_of_interconnection)

write_csv(aggregate_data_static,
          file =  paste0(project_root, "data/ic_studies/clean/project_static_scraped_final_data.csv"))
...
write.csv(aggregate_data_wide,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_final_data.csv"),
          row.names = FALSE)

write.csv(codebook_final,
          file = paste0(project_root, "data/ic_studies/clean/costs_phase_1_codebook_final.csv"),
          row.names = FALSE)



# 1) Pivot into long form and add flags
long_stats <- aggregate_data_wide %>%
  select(q_id, cost_network_shared, cost_network_leverage, cost_network_own) %>%
  pivot_longer(
    cols = c(cost_network_shared, cost_network_leverage),
    names_to  = "variable",
    values_to = "value"
  ) %>%
  mutate(
    is_nan = is.nan(value),
    is_inf = is.infinite(value),
    is_finite = is.finite(value)
  )

# 2) How many NaN/Inf and which q_id’s trigger Inf?
inf_rows <- long_stats %>%
  filter(is_inf) %>%
  select(q_id, variable, value, cost_network_own)

cat("Rows yielding Inf:\n")
print(inf_rows)

# 3) Summary stats over the finite values, plus counts of NaN/Inf
summary_stats <- long_stats %>%
  group_by(variable) %>%
  summarise(
    count_finite     = sum(is_finite),
    mean             = mean(value[is_finite],    na.rm = TRUE),
    median           = median(value[is_finite],  na.rm = TRUE),
    p75              = quantile(value[is_finite], 0.75, na.rm = TRUE),
    min              = min(value[is_finite],     na.rm = TRUE),
    max              = max(value[is_finite],     na.rm = TRUE),
    n_inf            = sum(is_inf),
    n_nan            = sum(is_nan),
    .groups = "drop"
  )
...
shared_bins <- aggregate_data_wide %>%
  mutate(
    shared_bin = factor(
      case_when(
        is.nan(cost_network_shared)               ~ "NaN",
      ...
      levels = c("NaN", "0-0.2", "0.2-0.5", "0.5-0.8", "0.8-1", "1+")
    )
  ) %>%
  count(shared_bin, name = "n_qid")
...
leverage_bins <- aggregate_data_wide %>%
  mutate(
    leverage_bin = factor(
      case_when(
        is.nan(cost_network_leverage)             ~ "NaN",
      ...
      levels = c("NaN","<1","1-2","2-5","5-10","10+")
    )
  ) %>%
  count(leverage_bin, name = "n_qid")

print(leverage_bins)

 

# 2) Check that cost_network_shared ∈ [0,1], excluding NaNs
clean_shared <- aggregate_data_wide %>%
  filter(!is.nan(cost_network_shared) & !is.na(cost_network_shared)) 

out_of_bounds <- clean_shared %>%
  filter(cost_network_shared < 0 | cost_network_shared > 1)

if (nrow(out_of_bounds) == 0) {
  cat("\ncost_network_shared is always between 0 and 1 (excluding NaNs).\n")
} else {
  cat("\nWARNING: these rows have cost_network_shared outside [0,1]:\n")
  print(out_of_bounds %>% select(q_id, cost_network_shared))
}


# Define x‐limits for each variable
x_limits_list <- list(
  cost_network_shared    = c(0, 1),
  cost_network_leverage  = c(1, 23)
)

# Loop over the two vars
for (var in names(x_limits_list)) {
  df <- aggregate_data_wide %>%
    filter(!is.nan(.data[[var]]) & !is.na(.data[[var]]))
  
  p <- ggplot(df, aes(x = .data[[var]])) +
    geom_histogram(aes(y = ..density..), bins = 25, alpha = 0.5) +
    geom_density() +
    coord_cartesian(xlim = x_limits_list[[var]]) + ylim(0,1)+
    labs(
...
# 4) For any NaNs, confirm cost_network_own == 0 (but only print failures, or a single success message)
nan_rows <- aggregate_data_wide %>%
  filter(is.nan(cost_network_shared) | is.nan(cost_network_leverage)) %>%
  mutate(own_zero = cost_network_own == 0)

if (nrow(nan_rows) == 0) {
  cat("No NaNs in cost_network_shared or cost_network_leverage.\n")
} else {
  if (all(nan_rows$own_zero)) {
    cat("For all", nrow(nan_rows), "rows with NaN, cost_network_own == 0.\n")
  } else {
    cat("These rows have NaNs but cost_network_own != 0:\n")
    print(
      nan_rows %>%
        filter(!own_zero) %>%
        select(q_id, cost_network_own, cost_network_shared, cost_network_leverage)
    )
  }
}
